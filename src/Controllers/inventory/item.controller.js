...
const createItem = asyncHandler(async (req, res, next) => {
  try {
    const { item_name, unit_id, item_unit, p_price, s_price, c_user } =
      req.body;
    if (
    ...
    const response =
      await query(`INSERT INTO items ( item_name, unit_id, item_unit, p_price, s_price, c_user)
                   VALUES ('${item_name}', '${unit_id}', '${item_unit}', '${p_price}', '${s_price}', '${c_user}')       
        `);
    res
      .status(201)
      .json(
